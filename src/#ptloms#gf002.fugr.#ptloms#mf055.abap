FUNCTION /ptloms/mf055.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
*"     REFERENCE(IM_TEXTO_LONGO) TYPE  STRING
*"  TABLES
*"      IT_TEXTO STRUCTURE  BAPI2080_NOTFULLTXTI
*"----------------------------------------------------------------------
...
* Declaração de tabela interna
  DATA: lt_status     TYPE TABLE OF string,
        lt_text_lines	TYPE STANDARD TABLE OF bapi2080_notfulltxti.
...
*        ls_text_lines-tdformat = '*'.
        ls_text_lines-objtype    = 'QMEL'.
        ls_text_lines-text_line  = ls_status+lv_tam(lv_tam2).
    ...
    ELSE.
      ...
      CLEAR ls_text_lines.
      ls_text_lines-objtype    = 'QMEL'.
      ls_text_lines-format_col = '*'.
      ls_text_lines-text_line  = ls_status.
      APPEND ls_text_lines TO lt_text_lines.
    ENDIF.
  ENDLOOP.
