...
FORM modificar_colunas .
  ...
  PERFORM alterar_atributos_coluna
    USING:
           ...
           'ILART'           'Tipo de Atividade de Manutenção'(136) 'X'    space   10 'X' '' '' '' '' '' '',
           'TPLNR'           'Local de instalação'(022)             'X'    space   10 'X' '' '' '' '' '' '',
           'PLTXT'           'Desc.Local de inst.'(023)             'X'    space   10 '' '' '' '' '' '' '',
           'EQUNR'           'Nº equipamento'(024)                  'X'    space   12 'X' '' '' '' '' '' '',
           'EQKTX'           'Desc.Nº equipamento'(025)             'X'    space   10 'X' '' '' '' '' '' '',
           ...
           'DATA_OPER_INI_STR' '1'                                   ''     space   10 '' '' '' '' 'X' '' '',
           'DATA_OPER_FIM_STR' '1'                                   ''     space   10 '' '' '' '' 'X' '' '',
           ...
           'STORTDESC'         'Descrição do campo de localização'  ''     space   10 '' '' '' '' '' '' '',
           'MN_WKCTR_PLANT'    'Centro do centro de trabalho'       ''     space   10 '' '' '' '' '' '' '',
           'MN_WKCTR_DESCRICAO' 'Descrição do centro de trabalho'   ''     space   10 '' '' '' '' '' '' ''.
...
*  CLEAR ls_fieldcatalog.
*  ls_fieldcatalog-ref_table = 'GT_DESPACHO_TREE'.
*  ls_fieldcatalog-fieldname = 'ILART'.
*  ls_fieldcatalog-coltext   = text-136.
*  ls_fieldcatalog-just      = 'C'.
*  ls_fieldcatalog-outputlen = '10'.
*  ls_fieldcatalog-no_out    = ' '.
*  APPEND ls_fieldcatalog TO gt_fieldcatalog.
*
  ...
  ls_fieldcatalog-outputlen = '20'.
  ls_fieldcatalog-no_out    = ' '.
*  ls_fieldcatalog-no_zero   = abap_true.
  APPEND ls_fieldcatalog TO gt_fieldcatalog.
...
*   Imagem ao expandir
  ...
  IF  gt_despacho_tree[]   IS NOT INITIAL.
* Verifica existenica de usuario sem correspondencia na tabela GT_NODES
    DATA: es_tree  LIKE LINE OF gt_despacho_tree,
          lt_nodes TYPE STANDARD TABLE OF /ptloms/cl008=>ty_nodes,
          es_nodes LIKE LINE OF lt_nodes.

    LOOP AT gt_despacho_tree   INTO es_tree.

      IF  es_tree-usuario IS NOT INITIAL.
        MOVE es_tree-usuario  TO ls_nodes-usuario.
    ...
    IF  lt_nodes[] IS NOT INITIAL.
      SORT lt_nodes.
      DELETE ADJACENT DUPLICATES FROM lt_nodes COMPARING ALL FIELDS.
      LOOP AT  gt_nodes   INTO es_nodes.
        READ TABLE lt_nodes  TRANSPORTING NO FIELDS WITH KEY usuario = es_nodes-usuario.
        IF  sy-subrc     NE 0.
          DELETE gt_nodes.
        ENDIF.
      ENDLOOP.
...
*   Imagem ao expandir
  ...
  CALL FUNCTION 'CONVERSION_EXIT_ALPHA_OUTPUT'
    EXPORTING
      input  = ps_despacho_tree-aufnr
    IMPORTING
      output = ps_despacho_tree-aufnr.
...
* Monta OBJECTTYPE
  lv_objecttype = p_aufnr && lv_vornr && lv_suboper.
...
FORM tira_zeros .
  ...
  LOOP AT  gt_despacho   INTO ls_despacho.
    ...
    CALL FUNCTION 'CONVERSION_EXIT_ALPHA_OUTPUT'
      EXPORTING
        input  = ls_despacho-equnr
      IMPORTING
        output = ls_despacho-equnr.

    MODIFY gt_despacho   FROM ls_despacho TRANSPORTING aufnr equnr.
...
*  <--  p2        text
*----------------------------------------------------------------------*
FORM tira_zeros_tree .

  LOOP AT  gt_despacho_tree   INTO ls_despacho_tree.
    ...
    CALL FUNCTION 'CONVERSION_EXIT_ALPHA_OUTPUT'
      EXPORTING
        input  = ls_despacho_tree-equnr
      IMPORTING
        output = ls_despacho_tree-equnr.

    MODIFY gt_despacho_tree   FROM ls_despacho_tree TRANSPORTING aufnr equnr.
